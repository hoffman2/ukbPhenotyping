...
dataDictionary <- fread("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/dependencies/Data_Dictionary_Showcase.csv",header = T)
dataCoding <- fread("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/dependencies/Codings_Showcase.csv")
phewasDefinition <- fread(file="/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/dependencies/Anno_HESelf_BI.csv",header=T)[,c(1,4)]
...
priorityFieldList <- fread("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/dependencies/priorityFieldList.ls", header=FALSE, sep="\t")
...
dataCodingWithNeg5 <- fread("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/dependencies/neg5Missing.ls",header = F)
dataCodingWithNeg3 <- fread("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/dependencies/neg3Missing.ls",header = F)
dataCodingWithNeg2 <- fread("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/dependencies/neg2Missing.ls",header = F)
dataCodingWithNeg1 <- fread("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/dependencies/neg1Missing.ls",header = F)
#Recode binary phenotypes to coding rules assigned by RS and JH

#Recode 90; Never,Past,Current(Coded past and current as 1)
#First create a new variable for tobacco smoke that keeps it as a categorical variable
bd[,f_20116_0_0_CAT_never_former_current_smoking_status:=f.20116.0.0]
...
bd[,f.3062.0.0 :=ifelse(f.3061.0.0==0|f.3061.0.0==32,f.3062.0.0,NA)]
bd[,f.3062.0.1 :=ifelse(f.3061.0.1==0|f.3061.0.1==32,f.3062.0.1,NA)]
bd[,f.3062.0.2 :=ifelse(f.3061.0.2==0|f.3061.0.2==32,f.3062.0.2,NA)]
...
bd[,f.3062.0_f_QUANT_FVC_maximumValue_strict:=ifelse(f.20152.0.0==1,f.3062.0_f_QUANT_FVC_maximumValue,NA)]
...
bd[,f.3063.0.0 :=ifelse(f.3061.0.0==0|f.3061.0.0==32,f.3063.0.0,NA)]
bd[,f.3063.0.1 :=ifelse(f.3061.0.1==0|f.3061.0.1==32,f.3063.0.1,NA)]
bd[,f.3063.0.2 :=ifelse(f.3061.0.2==0|f.3061.0.2==32,f.3063.0.2,NA)]
...
#Also included 20152 which defines reproducibility of best measure
bd[,f.3063.0_f_QUANT_FEV1_maximumValue_strict:=ifelse(f.20152.0.0==1,f.3063.0_f_QUANT_FEV1_maximumValue,NA)]

#Take maximum of first instance Field 3064
#Also included 20152 which defined reproducibility of best measure
bd[,f.3064.0.0 :=ifelse(f.3061.0.0==0|f.3061.0.0==32,f.3064.0.0,NA)]
bd[,f.3064.0.1 :=ifelse(f.3061.0.1==0|f.3061.0.1==32,f.3064.0.1,NA)]
bd[,f.3064.0.2 :=ifelse(f.3061.0.2==0|f.3061.0.2==32,f.3064.0.2,NA)]
...
bd[,f.3064.0_f_QUANT_PEF_maximumValue_strict:=ifelse(f.20152.0.0==1,f.3064.0_f_QUANT_PEF_maximumValue,NA)]
#####################################################################################################################################
...
#####################################################################################################################################
#Derived my own ERS/ATS variable for blow reproducibility (same code as above)
#Need second highest number after max fev
# bd[,f.3063.0_f_QUANT_FEV1_2nd_maximumValue := apply(select(bd,starts_with("f.3063.0.")),1,function(x) {sort(x,decreasing=T)[][2]})]
# set(bd,i=which(is.infinite(bd[["f.3063.0_f_QUANT_FEV1_2nd_maximumValue"]])),j="f.3063.0_f_QUANT_FEV1_2nd_maximumValue",value=NA)
# bd[,f.20152.0_f_BIN_derived_ERS_ATS := ifelse(f.3063.0_f_QUANT_FEV1_maximumValue>=1&(f.3063.0_f_QUANT_FEV1_maximumValue-f.3063.0_f_QUANT_FEV1_2nd_maximumValue)<=.150 & !is.na(f.3063.0_f_QUANT_FEV1_2nd_maximumValue)&!is.na(f.3063.0_f_QUANT_FEV1_maximumValue),1,
#   ifelse((f.3063.0_f_QUANT_FEV1_maximumValue < 1 & (f.3063.0_f_QUANT_FEV1_maximumValue-f.3063.0_f_QUANT_FEV1_2nd_maximumValue)<=.100)&!is.na(f.3063.0_f_QUANT_FEV1_2nd_maximumValue)&!is.na(f.3063.0_f_QUANT_FEV1_maximumValue),1,0))]
...
bd[,f.102.0_f_QUANT_pulse_minimumValue := apply(select(bd,starts_with("f.102.0")),1,min,na.rm=T)]
set(bd,i=which(is.infinite(bd[["f.102.0_f_QUANT_pulse_minimumValue"]])),j="f.102.0_f_QUANT_pulse_minimumValue",value=NA)
...
for(currentField in dataDictionary[dataDictionary$Coding %in% "100539"]$FieldID){
  fieldArray= grep(x=names(bd),pattern=(paste0("^f\\.",currentField,"\\.")),value=TRUE)
  for(field in fieldArray) set(bd, i=which(bd[[field]]==-3|bd[[field]]==-1), j=field, value=NA)}
...
for(currentField in dataDictionary[dataDictionary$Coding %in% "272"]$FieldID){
  ...
  #Even though date is missing, we would like to still keep as case( commenting out line below)
...
#100635 (Zero is coded correctly. Ommited from numbers below)
for(currentField in dataDictionary[dataDictionary$Coding %in% "100635"]$FieldID){
  fieldArray= grep(x=names(bd),pattern=(paste0("^f\\.",currentField,"\\.")),value=TRUE)
  for(field in fieldArray) set(bd, i=which(bd[[field]]==11), j=field, value=1)
  for(field in fieldArray) set(bd, i=which(bd[[field]]==12), j=field, value=1)
  for(field in fieldArray) set(bd, i=which(bd[[field]]==13), j=field, value=1)
  for(field in fieldArray) set(bd, i=which(bd[[field]]==14), j=field, value=1)
  for(field in fieldArray) set(bd, i=which(bd[[field]]==-1|bd[[field]]==-3), j=field, value=NA)}
...
#100696
for(currentField in dataDictionary[dataDictionary$Coding %in% "100696"]$FieldID){
  fieldArray= grep(x=names(bd),pattern=(paste0("^f\\.",currentField,"\\.")),value=TRUE)
  for(field in fieldArray) set(bd, i=which(bd[[field]]=="-1"), j=field, value=NA)}
...
# #field 6142 related to employement. Keeping it as a category since only to be used as covariate in pain phenotyping
# for(currentField in dataDictionary[dataDictionary$Coding %in% "100295"]$FieldID){
#   fieldArray= grep(x=names(bd),pattern=(paste0("^f\\.",currentField,"\\.")),value=TRUE)
#   for(field in fieldArray) set(bd, i=which(bd[[field]]==-3), j=field, value=NA)
#   for(field in fieldArray) set(bd, i=which(bd[[field]]==-7), j=field, value=0)}
...
#All pain is an exclusive phenotype that needs to be updated so that the other pain variables equal 1
fieldWide[fieldWide$`8`==1,c(2:ncol(fieldWide))]=1
#Remove the columns that are by-products of the coding
fieldWide[,c("-9","NA","0"):=NULL]
#Grab code descriptions
...
#Created new pain variable names based on self report using first instance but all arrays
#Select the subject ID and the relevant field thats needs to be aggregated
fieldArray=names(select(bd,starts_with("f.6142.0.")))
justFields <- bd[,c("f.eid",fieldArray),with=FALSE]
#For subjects that have NA for all columns then set them to missing with universal missingness code
set(justFields, i=which((ncol(justFields)-1)==rowSums(is.na(justFields))), j=fieldArray, value=-3)
#Take the arrayed field and put into long format, using the f.eid as the key(keyed values do not need to be explicit in statement)
fieldLong = melt(justFields, measure = patterns("^f.6142.0."), variable.name="arbField",value.name = c("dxField"))
#Create a new variable for the binary phenotyping of the individual data codes
fieldLong$DX=NA
fieldLong$DX=as.integer(fieldLong$DX)
#For any code >0 assign caseCode a value of 1
...
fieldWide[,c("-9","NA","0"):=NULL]
#Grab code descriptions
fieldNames <- dataCoding[Coding=="100295",]
names(fieldNames)[2:3]=c("V1","V2")
names(fieldWide)[2:length(names(fieldWide))]=paste0("code",names(fieldWide)[2:length(names(fieldWide))])
fieldNames$V1=paste0("code",fieldNames$V1)
#Create the abbreviated names list
fieldNames$V2=gsub("[[:punct:]]", "_",fieldNames$V2)
fieldNames$abbrev=abbreviate(fieldNames$V2,10)
for(currentName in names(fieldWide)[grep("code",names(fieldWide))]) setnames(fieldWide,names(fieldWide)[grep(paste0("\\b",currentName,"\\b"),names(fieldWide))],paste("f.6142",currentName,"BIN",filter(fieldNames,V1==currentName)["V2"],sep = "_"))
#Join these new columns to data.table and replace previous version
bd<-bd[fieldWide,nomatch="0",on="f.eid"]
...
fieldWide[,c("dxAge_-9","NA","0","-9","DX_-9"):=NULL]
...
for(currentName in names(fieldWide)[grep("DX",names(fieldWide))]) set(fieldWide, i=which(is.na(fieldWide[[currentName]])), j=currentName, value=0)
#Bug fix for subjects with all missing
#Not a bug, for nurses interview, if a subject reported no diseases then were considered a non-participant
...
operativeProcedures <- fread("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/dependencies/coding240.tsv")
...
fieldLong$DX=NA
fieldLong$DX=as.integer(fieldLong$DX)
#Use >1 since we are setting reference to 1
set(fieldLong, i=which(fieldLong[["dxField"]]>0), j="DX", value=1)
# -7 is not part of this questions coding but kept it in
set(fieldLong, i=which(fieldLong[["dxField"]]==-7), j=c("dxField","DX"), value=0)
set(fieldLong, i=which(fieldLong[["dxField"]]==-1|fieldLong[["dxField"]]==-3), j=c("DX","dxField"), value=-9)
fieldLong[,"arbField":=NULL]
fieldLong <- fieldLong[!duplicated(fieldLong,by=c("f.eid","dxField")),]
...
bd<-bd[fieldWide,nomatch="0",on="f.eid"]
#We decided to curate balding as 1 against everything else as an additional variable
bd[,f.2395.0.0:=ifelse(is.na(f.2395.0.0),NA,ifelse(f.2395.0.0==1,0,ifelse(f.2395.0.0==2,1,ifelse(f.2395.0.0==3,1,ifelse(f.2395.0.0==4,1,NA)))))]
######################################################################################################################################################
...
hesDataPrim <- fread("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/dependencies/HESIN_26041_all.minus4226129.tsv",header=T,sep="\t",na.strings="")
hesDataSec <- fread("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/dependencies/HESIN_SECONDARY_DIAG10_26041_all.minus508567.tsv",header=T,sep="\t",na.strings="")
#PheWasDefinitions
...
#######################################################################################################################################
#HES Data Primary Only
print("Aggregate HES data for primary codes only")
#HES Data
#Read in the primary and seconday diagnosis files that were previously generated by Robert Scott
hesDataPrim <- fread("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/dependencies/HESIN_26041_all.minus4226129.tsv",header=T,sep="\t",na.strings="")
#PheWasDefinitions
...
names(hesDataPrim)[5]="diag"
names(hesDataSec)[4]="diag"
#Only use hesPrim but renaming to combine to reduce variable name changes
hesDataCombined<-rbind(hesDataPrim[,c(1,5)])
#Add "p_" to each variablte)
#Roll up codes based on left three characters
#HES data is read in as long format so some of the steps used in previous sections not necessary
#For now we are rolling up ICD10 codes into phewas codes by using first 3 characters and renaming with"p_"
...
for(currentName in names(fieldWide)) set(fieldWide, i=which(is.na(fieldWide[[currentName]])), j=currentName, value=0)
#Replace names
for(currentName in names(fieldWide)[grep("p_",names(fieldWide))]) setnames(fieldWide,names(fieldWide)[grep(paste0("^",currentName,"$"),names(fieldWide))],paste("HES_primary_",currentName,"BIN",filter(phewasDefinition,pheno==currentName)["PHENOTYPE"],sep = "_"))
#Perform left join and replace previous version
setkey(fieldWide,f.eid)
setkey(bd,f.eid)
#This is a merge tat takes advantage of data.table although the syntax suggests otherwise
...
fieldNames<-fread(file="/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/dependencies/Anno_HESelf_BI.csv",header=T)[,c(1,4)]
...
myNamingScheme <- fread("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/dependencies/namingFile.keepFirst.priority.txt")
...
#####################################################################################################################################
#Following code is depreacted due to updates by RWD
...
#####################################################################################################################################
####################################################################################################################################
#RWD performed an update to the two way mapping
#Slightly different form to how the mapping file was created.
#In the new file there is no formal reference to whether code is roled up or not. I performed some steps decribed below in excel to generate a file to perform the mapping process
#In "UKBB_Ill_2_ICD10_mapped_RS_CT_JDH" I creataed a new sheet and pasted in columns E-H
#In column E(LENGTH); I inserted length of column C: =LEN(C2)
#In column F(3CHAR); I took the left 3 characters from column C: =LEFT(C2,3)
#I then sorted the spreadsheet using the "Sort command": 1) Code,Values,Smallest to Largest 2) 3CHAR,Values,A to Z 3) LENGTH, Values, Smallest to Largest
#In column G(IS_IT_ROOT); I created a variable to flag codes that had duplicate 3 digit HES and NI codes. Seting the first code in the duplicate set to 0 and the following dupes to 1: =IF(AND(A2=A1,F2=F1),0,1)
#In column H(3CHAR_ROOT); I flagged 3 digit root codes(base code was 3 digit and the following codes were 4 or 5 digits): =IF(AND(A2<>A1,E2=3),1,0)
#Column I(4CHAR_ROOT); I flagged 4 digit root codes: =IF(AND($A2<>$A1,$E2=4),1,0)
#Column J(5CHAR_ROOT); 5 digit root codes: =IF(AND($A2<>$A1,$E2=5),1,0)
#Column K(ROOT_LENGTH); I introduced a flag to give the character length of the actual root code. First selected on rows with column G=1;=E2
#Column L(FINAL_ROOT); Return the root HES code for each row by using the root length calculated in column K: =LEFT(C2,K2)
#I then pasted Columns A,B,C,L into a new sheet
####################################################################################################################################

#RWD has reformated the 2 way mapping with new mappings as well
twoWayMapping <- fread("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/UKBB_Ill_2_ICD10_mapped_RS_CT_JDH.txt",header=T,colClasses="character")
twoWayMapping$ICD10=gsub("[[:punct:]]", "",twoWayMapping$ICD10)
twoWayMapping$Code_Description=gsub("[[:punct:]]", "",twoWayMapping$Code_Description)
twoWayMapping$Code_Description=gsub("[[:punct:]]", "_",twoWayMapping$Code_Description)
twoWayMapping$Code_Description=gsub(" ", "_",twoWayMapping$Code_Description)
twoWayMapping$Code_Description=gsub("_{1,}", "_",twoWayMapping$Code_Description)
twoWayMapping$Code_Description=gsub(" {1,}", "_",twoWayMapping$Code_Description)
twoWayMapping$Code_Description=gsub("_$", "",twoWayMapping$Code_Description)
hesDataPrim <- fread("/GWD/appbase/projects/RD-TSci-PhewasUKB/josh/phenotypes/HESIN_26041_all.minus4226129.tsv",header=T,sep="\t",na.strings="")
hesDataSec <- fread("/GWD/appbase/projects/RD-TSci-PhewasUKB/josh/phenotypes/HESIN_SECONDARY_DIAG10_26041_all.minus508567.tsv",header=T,sep="\t",na.strings="")
names(hesDataPrim)[5]="diag"
...
#fieldWide<-dcast(fieldLong, f.eid ~ dxField, value.var = c("DX"))
#for(currentName in names(fieldWide)) set(fieldWide, i=which(is.na(fieldWide[[currentName]])), j=currentName, value=0)
#ent due to no cases being identified, I went ahead and added in an additional column called "zeroColumn", where each row value is set to 0. 
#This will be used in the steps below where we need to perform ifelse statements. In situations where a code's column is not find, the zeroColumn will be used as a surrogate
#Again this is assuming that the reason that the HES code column is not present is because no cases where identified during wide formatting.
bd$zeroColumn=0
#Iterative process to perform the new two way mapping, instead of using an ifelse statement in wide format, I decided to use loop through each code individually and assign case status if code/person combination occured
{ #This block of code is for performing the new 2 way mapping with RWD data
for(interviewCode in unique(twoWayMapping$Code)) {
  print(interviewCode)
  fieldInterview=grep(x=names(bd),pattern=(paste0("^f\\.","20002\\.0_dxCode",interviewCode)),value=TRUE)
  #fieldInterview=grep(x=names(bd),pattern=(paste0("^f\\_","20002\\_0_dxCode",interviewCode)),value=TRUE)
  fieldInterview=ifelse(length(fieldInterview)==0,"zeroColumn",fieldInterview)
  bd[, paste0("map2way_NI_code",interviewCode):=0]
  set(bd,i=which(bd[[fieldInterview]]==1), j=paste0("map2way_NI_code",interviewCode), value=1)
  for(hesCode in twoWayMapping[Code %in% interviewCode,ICD10]){
    subjectsWithHES=fieldLong[dxField %in% paste0("p_",hesCode),][!duplicated(f.eid),f.eid]
    set(bd,i=which(bd$f.eid %in% subjectsWithHES), j=paste0("map2way_NI_code",interviewCode), value=1)
}}

#Now reduce 5 digit codes to 4 digit codes and repeat within the primary and secondary data
fieldLong$dxField_4char=substr(fieldLong$dxField,1,6) #put in 6 because we added the "p" and "_" characters
fieldLong <- fieldLong[!duplicated(fieldLong,by=c("f.eid","dxField_4char")),]
for(interviewCode in unique(twoWayMapping$Code)) {
  print(interviewCode)
  fieldInterview=grep(x=names(bd),pattern=(paste0("^f\\.","20002\\.0_dxCode",interviewCode)),value=TRUE)
  #fieldInterview=grep(x=names(bd),pattern=(paste0("^f\\_","20002\\_0_dxCode",interviewCode)),value=TRUE)
  ...
  set(bd,i=which(bd[[fieldInterview]]==1), j=paste0("map2way_NI_code",interviewCode), value=1)
  for(hesCode in twoWayMapping[Code %in% interviewCode,ICD10]){
    subjectsWithHES=fieldLong[dxField_4char %in% paste0("p_",hesCode),][!duplicated(f.eid),f.eid]
    set(bd,i=which(bd$f.eid %in% subjectsWithHES), j=paste0("map2way_NI_code",interviewCode), value=1)
}}
#Now reduce 5 digit codes to 3 digit codes and repeat within the primary and secondary data
fieldLong$dxField_3char=substr(fieldLong$dxField,1,5) #put in 6 because we added the "p" and "_" characters
fieldLong <- fieldLong[!duplicated(fieldLong,by=c("f.eid","dxField_3char")),]
for(interviewCode in unique(twoWayMapping$Code)) {
  print(interviewCode)
  fieldInterview=grep(x=names(bd),pattern=(paste0("^f\\.","20002\\.0_dxCode",interviewCode)),value=TRUE)
  #fieldInterview=grep(x=names(bd),pattern=(paste0("^f\\_","20002\\_0_dxCode",interviewCode)),value=TRUE)
  ...
  set(bd,i=which(bd[[fieldInterview]]==1), j=paste0("map2way_NI_code",interviewCode), value=1)
  for(hesCode in twoWayMapping[Code %in% interviewCode,ICD10]){
    subjectsWithHES=fieldLong[dxField_3char %in% paste0("p_",hesCode),][!duplicated(f.eid),f.eid]
    set(bd,i=which(bd$f.eid %in% subjectsWithHES), j=paste0("map2way_NI_code",interviewCode), value=1)
}}
}
#Need to create identifier for mapping file to add description
twoWayMapping$mappingID=paste0("map2way_NI_code",twoWayMapping$Code)
twoWayMapping_forAddingDescr <- twoWayMapping[!duplicated(mappingID),]
for(currentName in names(bd)[grep("map2way_NI_code",names(bd))]) setnames(bd,names(bd)[grep(paste0("^",currentName,"$"),names(bd))],paste(currentName,"BIN",twoWayMapping_forAddingDescr[mappingID %in% currentName,Code_Description],sep = "_"))


#Create total fracture group using 2 way mappings
...
#Deprecated due to new RWD mapping( incorpareted some into new 3 way mapping)
# #DVT
code1=grep(x=names(bd),pattern=(paste0("^f\\.6152\\.0_code5")),value=TRUE)
code1=ifelse(length(code1)==0,"zeroColumn",code1)
code2="map2way_NI_code1094_BIN_deep_venous_thrombosis_dvt"
...
#bd[, map3way_f6152_code5_intCode1094_BIN_DVT_group_cardiovascular:=ifelse(bd[[code1]]==1|bd[[code2]]==1,1,ifelse(bd[[code1]]==0 & bd[[code2]]==0,0,NA))]
...
code2="map2way_NI_code1113_BIN_emphysemachronic_bronchitis"
# code2=ifelse(length(code2)==0,"zeroColumn",code2)
# code3=grep(x=names(bd),pattern="^f\\.20002\\.0_dxCode1412",value=TRUE)
code3="map2way_NI_code1412_BIN_bronchitis"
# code3=ifelse(length(code3)==0,"zeroColumn",code3)
# code4=grep(x=names(bd),pattern="^f\\.20002\\.0_dxCode1472",value=TRUE)
code4="map2way_NI_code1472_BIN_emphysema"
# code4=ifelse(length(code4)==0,"zeroColumn",code4)
...
#bd[, map3way_f6152_code6_intCode1113_1412_1472_f22129_BIN_emphysema_bronchitis_group_respiratory:=ifelse((bd[[code1]]==0|is.na(bd[[code1]])) & bd[[code2]]==0 & bd[[code3]]==0 & bd[[code4]]==0 & (bd[[code8]]== 0|is.na(bd[[code8]])),0,ifelse(bd[[code1]]==1|bd[[code2]]==1|bd[[code3]]==1|bd[[code4]]==1|bd[[code8]]==1,1,NA))]

# #pulmonary_embolism
code1=grep(x=names(bd),pattern="^f\\.6152\\.0_code7",value=TRUE)
code1=ifelse(length(code1)==0,"zeroColumn",code1)
code2="map2way_NI_code1093_BIN_pulmonary_embolism_dvt"
...
#bd[, map3way_f6152_code7_intCode1093_pulmonary_BIN_embolism:=ifelse(bd[[code1]]==1|bd[[code2]]==1,1,ifelse(bd[[code1]]==0 & bd[[code2]]==0,0,NA))]

# #asthma
code1=grep(x=names(bd),pattern="^f\\.6152\\.0_code8",value=TRUE)
code1=ifelse(length(code1)==0,"zeroColumn",code1)
code2="map2way_NI_code1111_BIN_asthma"
...
#bd[, map3way_f6152_code8_intCode1111_f22127_BIN_asthma_group_respiratory:=ifelse((bd[[code1]]==0|is.na(bd[[code1]])) & bd[[code2]]==0 & (bd[[code8]]==0|is.na(bd[[code8]])),0,ifelse(bd[[code1]]==1|bd[[code2]]==1|bd[[code8]]==1,1,NA))]

# #hay_fever_eczema
code1=grep(x=names(bd),pattern="^f\\.6152\\.0_code9",value=TRUE)
code1=ifelse(length(code1)==0,"zeroColumn",code1)
code2="map2way_NI_code1387_BIN_hayfeverallergic_rhinitis"
# code2=grep(x=names(bd),pattern="^f\\.20002\\.0_dxCode1387",value=TRUE)
# code2=ifelse(length(code2)==0,"zeroColumn",code2)
code3="map2way_NI_code1452_BIN_eczemadermatitis"
...
#bd[, map3way_f6152_code9_intCode1387_1452_f22126_BIN_hay_fever_eczema_group_immunological:=ifelse((bd[[code1]]==0|is.na(bd[[code1]])) & bd[[code2]]==0 & bd[[code3]]==0 & (bd[[code8]]== 0|is.na(bd[[code8]])),0,ifelse(bd[[code1]]==1|bd[[code2]]==1|bd[[code3]]==1|bd[[code8]]==1,1,NA))]

# #myocardial_infarction
code1=grep(x=names(bd),pattern="^f\\.6150\\.0_code1",value=TRUE)
code1=ifelse(length(code1)==0,"zeroColumn",code1)
code2="map2way_NI_code1075_BIN_heart_attackmyocardial_infarction"
...
#bd[, map3way_f6150_code1_intCode1075_BIN_myocardial_infarction_group_cardiovascular:=ifelse(bd[[code1]]==1|bd[[code2]]==1,1,ifelse(bd[[code1]]==0 & bd[[code2]]==0,0,NA))]

# #angina
code1=grep(x=names(bd),pattern="^f\\.6150\\.0_code2",value=TRUE)
code1=ifelse(length(code1)==0,"zeroColumn",code1)
code2="map2way_NI_code1074_BIN_angina"
...
#bd[, map3way_f6150_code2_intCode1074_BIN_angina_group_cardiovascular:=ifelse(bd[[code1]]==1|bd[[code2]]==1,1,ifelse(bd[[code1]]==0 & bd[[code2]]==0,0,NA))]

# #stroke
code1=grep(x=names(bd),pattern="^f\\.6150\\.0_code3",value=TRUE)
code1=ifelse(length(code1)==0,"zeroColumn",code1)
code2="map2way_NI_code1081_BIN_stroke"
...
#bd[, map3way_f6150_code3_intCode1081_BIN_stroke_group_cardiovascular:=ifelse(bd[[code1]]==1|bd[[code2]]==1,1,ifelse(bd[[code1]]==0 & bd[[code2]]==0,0,NA))]

# #hypertension
code1=grep(x=names(bd),pattern="^f\\.6150\\.0_code4",value=TRUE)
code1=ifelse(length(code1)==0,"zeroColumn",code1)
code2="map2way_NI_code1065_BIN_hypertension"
# code2=grep(x=names(bd),pattern="^f\\.20002\\.0_dxCode1065",value=TRUE)
# code2=ifelse(length(code2)==0,"zeroColumn",code2)
code3="map2way_NI_code1072_BIN_essential_hypertension"
...
#bd[, map3way_f6150_code4_intCode1065_1072_BIN_hypertension_group_cardiovascular:=ifelse(bd[[code1]]==1|bd[[code2]]==1|bd[[code3]]==1,1,ifelse(bd[[code1]]==0 & bd[[code2]]==0 & bd[[code3]]==0,0,NA))]

# #gestational_diabetes
code1=grep(x=names(bd),pattern="^f\\.4041\\.0",value=TRUE)
code1=ifelse(length(code1)==0,"zeroColumn",code1)
code2="map2way_NI_code1221_BIN_gestational_diabetes"
...
#bd[, map3way_f4041_intCode1221_BIN_gestational_diabetes_group_endocrine:=ifelse((bd[[code1]]==0|is.na(bd[[code1]])) & bd[[code2]]==0,0,ifelse(bd[[code1]]==1|bd[[code2]]==1,1,NA))]
...
#Create a variable for each block, setting everyone initially as a control
...
for(blockHES in hesOntology[!duplicated(CAT2)]$CAT2){
  bd[,paste0("HES_block_p_",blockHES,"_BIN") := 0]
  if(ncol(bd[,names(bd) %in% hesOntology[CAT2 %in% blockHES,hesCode],with=F])==0)
    print(paste0(blockHES,":is null"))
  else(set(bd,i=which(apply(bd[,names(bd) %in% hesOntology[CAT2 %in% blockHES,hesCode],with=F]==1,1,any)),j=paste0("HES_block_p_",blockHES,"_BIN"),value=1))}
#Chapter
for(chapterHES in hesOntology[!duplicated(CAT1)]$CAT1){
  bd[,paste0("HES_chapter_p_",chapterHES,"_BIN") := 0]
  if(ncol(bd[,names(bd) %in% hesOntology[CAT1 %in% chapterHES,hesCode],with=F])==0)
    print(paste0(chapterHES,":is null"))
  else(set(bd,i=which(apply(bd[,names(bd) %in% hesOntology[CAT1 %in% chapterHES,hesCode],with=F]==1,1,any)),j=paste0("HES_chapter_p_",chapterHES,"_BIN"),value=1))}
...
# # #For each block, iterate through all the original phewas codes that make up that block and assign case status to that block variable if "1" is present in PheWas code
# for(blockHES in hesOntology[CAT2 %like% "^C|^D|^O0"][!duplicated(CAT2)]$CAT2) {phenoList=hesOntology[CAT2==blockHES,"cancerRegVar"];for(phenoVariable in phenoList$cancerRegVar){set(bd, i=which(bd[[phenoVariable]]==1), j=paste0("f_40006_block_",blockHES,"_BIN"), value=1)}}
# # #Chapter variable
# for(chapterHES in hesOntology[CAT2 %like% "^C|^D|^O0"][!duplicated(CAT1)]$CAT1) {bd[,paste0("f_40006_chapter_",chapterHES,"_BIN") := 0]}
# for(chapterHES in hesOntology[CAT2 %like% "^C|^D|^O0"][!duplicated(CAT1)]$CAT1) {phenoList=hesOntology[CAT1==chapterHES,"cancerRegVar"];for(phenoVariable in phenoList$cancerRegVar){set(bd, i=which(bd[[phenoVariable]]==1), j=paste0("f_40006_chapter_",chapterHES,"_BIN"), value=1)}}
#Coded slightly differently
for(blockCancer in hesOntology[CAT2 %like% "^C|^D|^O0"][!duplicated(CAT2)]$CAT2){
  bd[,paste0("f_40006_block_",blockCancer,"_BIN") := 0]
  if(ncol(bd[,names(bd) %in% hesOntology[CAT2 %in% blockCancer,cancerRegVar],with=F])==0)
    print(paste0(blockCancer,":is null"))
  else(set(bd,i=which(apply(bd[,names(bd) %in% hesOntology[CAT2 %in% blockCancer,cancerRegVar],with=F]==1,1,any)),j=paste0("f_40006_block_",blockCancer,"_BIN"),value=1))}
#Chapter
for(chapterCancer in hesOntology[CAT2 %like% "^C|^D|^O0"][!duplicated(CAT1)]$CAT1){
  bd[,paste0("f_40006_chapter_",chapterCancer,"_BIN") := 0]
  if(ncol(bd[,names(bd) %in% hesOntology[CAT1 %in% chapterCancer,cancerRegVar],with=F])==0)
    print(paste0(chapterCancer,":is null"))
  else(set(bd,i=which(apply(bd[,names(bd) %in% hesOntology[CAT1 %in% chapterCancer,cancerRegVar],with=F]==1,1,any)),j=paste0("f_40006_chapter_",chapterCancer,"_BIN"),value=1))}
#######################################################################################################################################
#Special Requests
#LVEF was left raw
...
#With ERS_ATS criteria
bd[,specialRequest_QUANT_FEV1_FVC_ratio_strict := ifelse(f_20152_0_0_f_BIN_Reproduciblity_of_spirometry_measurement_using_ERS_ATS_criteria==1,specialRequest_QUANT_FEV1_FVC_ratio,NA)]
#Same ratio but with martin tobin best measure
bd[,specialRequest_QUANT_FEV1_FVC_ratio_martin_tobin_derived := f_20150_0_0_f_QUANT_Forced_expiratory_volume_in_1_second_FEV1_Best_measure/f_20151_0_0_f_QUANT_Forced_vital_capacity_FVC_Best_measure]
#With ERS_reproducibility
bd[,f_20154_0_0_f_QUANT_Forced_expiratory_volume_in_1_second_FEV1_predicted_percentage_strict:=ifelse(f_20152_0_0_f_BIN_Reproduciblity_of_spirometry_measurement_using_ERS_ATS_criteria==1,f_20154_0_0_f_QUANT_Forced_expiratory_volume_in_1_second_FEV1_predicted_percentage,NA)]
bd[,specialRequest_QUANT_FEV1_FVC_ratio_martin_tobin_derived_strict := ifelse(f_20152_0_0_f_BIN_Reproduciblity_of_spirometry_measurement_using_ERS_ATS_criteria==1,specialRequest_QUANT_FEV1_FVC_ratio_martin_tobin_derived,NA)]
#Martin Tobin best measure with reproducibility
bd[,f_20150_0_0_f_QUANT_Forced_expiratory_volume_in_1_second_FEV1_Best_measure_strict := ifelse(f_20152_0_0_f_BIN_Reproduciblity_of_spirometry_measurement_using_ERS_ATS_criteria==1,f_20150_0_0_f_QUANT_Forced_expiratory_volume_in_1_second_FEV1_Best_measure,NA)]
bd[,f_20151_0_0_f_QUANT_Forced_vital_capacity_FVC_Best_measure_strict := ifelse(f_20152_0_0_f_BIN_Reproduciblity_of_spirometry_measurement_using_ERS_ATS_criteria==1,f_20151_0_0_f_QUANT_Forced_vital_capacity_FVC_Best_measure,NA)]
#Same for the quantitative spirometry measures
bd[,f_3062_0_f_QUANT_FVC_maximumValue_strict := ifelse(f_20152_0_0_f_BIN_Reproduciblity_of_spirometry_measurement_using_ERS_ATS_criteria==1,f_3062_0_f_QUANT_FVC_maximumValue,NA)]
bd[,f_3063_0_f_QUANT_FEV1_maximumValue_strict := ifelse(f_20152_0_0_f_BIN_Reproduciblity_of_spirometry_measurement_using_ERS_ATS_criteria==1,f_3063_0_f_QUANT_FEV1_maximumValue,NA)]
bd[,f_3064_0_f_QUANT_PEF_maximumValue_strict := ifelse(f_20152_0_0_f_BIN_Reproduciblity_of_spirometry_measurement_using_ERS_ATS_criteria==1,f_3064_0_f_QUANT_PEF_maximumValue,NA)]
...
# Joint pain
bd[,specialRequest_BIN_joint_pain := ifelse(bd[["f_3414_0_0_f_BIN_Hip_pain_for_3_months"]]==1|bd[["f_3404_0_0_f_BIN_Neck_shoulder_pain_for_3_months"]]==1|bd[["f_3773_0_0_f_BIN_Knee_pain_for_3_months"]]==1,1,ifelse(bd[["f_3414_0_0_f_BIN_Hip_pain_for_3_months"]]==0 & bd[["f_3404_0_0_f_BIN_Neck_shoulder_pain_for_3_months"]]==0&bd[["f_3773_0_0_f_BIN_Knee_pain_for_3_months"]]==0,0,NA))]
...
bd[,specialRequest_QUANT_FEV1_never_smoked_only_strict:= ifelse(f_20152_0_0_f_BIN_Reproduciblity_of_spirometry_measurement_using_ERS_ATS_criteria==1,specialRequest_QUANT_FEV1_never_smoked_only,NA)]
...
bd[,specialRequest_QUANT_FEV1_ever_smoked_only_strict:= ifelse(f_20152_0_0_f_BIN_Reproduciblity_of_spirometry_measurement_using_ERS_ATS_criteria==1,specialRequest_QUANT_FEV1_ever_smoked_only,NA)]
...
bd[,specialRequest_QUANT_FEV1_FVC_ratio_never_smoked_only_strict:= ifelse(f_20152_0_0_f_BIN_Reproduciblity_of_spirometry_measurement_using_ERS_ATS_criteria==1,specialRequest_QUANT_FEV1_FVC_ratio_never_smoked_only,NA)]
...
bd[,specialRequest_QUANT_FEV1_FVC_ratio_ever_smoked_only_strict:= ifelse(f_20152_0_0_f_BIN_Reproduciblity_of_spirometry_measurement_using_ERS_ATS_criteria==1,specialRequest_QUANT_FEV1_FVC_ratio_ever_smoked_only,NA)]
#Predicted FEV1
...
#Set subjects with a non-reproducible 20152 score to NA (The 3063 variable mentioned below incorporates the 20152 flag)
...
#Strict FEV1 predicted percentage based on 20152:(The 3063 variable mentioned below incorporates the 20152 flag)
...
###########################################################################################################################
#COPD as defined by spirometry and self report (Based on discussions with Mathias, Dan, and Soumitra)
...
hesDataAsthmaExacPrimSecondary <- hesDataCombined[diag %like% "J45",]
hesDataAsthmaExac <- hesDataAsthmaExac[!duplicated(hesDataAsthmaExac,by=c("eid")),]
hesDataAsthmaExacPrimSecondary <- hesDataAsthmaExacPrimSecondary[!duplicated(hesDataAsthmaExacPrimSecondary,by=c("eid")),]

hesDataCOPDExacPrim[,copd_prim:=1]
hesDataCOPDExacPrimSecondary[,copd_prim_second:=1]
hesDataAsthmaExac[,asthma_prim:=1]
hesDataAsthmaExacPrimSecondary[,asthma_prim_second:=1]
bd <- merge(bd,hesDataCOPDExacPrim[,c("eid","copd_prim")],by.x="f_eid",by.y="eid",all.x=T)
bd <- merge(bd,hesDataAsthmaExac[,c("eid","asthma_prim")],by.x="f_eid",by.y="eid",all.x=T)
bd <- merge(bd,hesDataCOPDExacPrimSecondary[,c("eid","copd_prim_second")],by.x="f_eid",by.y="eid",all.x=T)
bd <- merge(bd,hesDataAsthmaExacPrimSecondary[,c("eid","asthma_prim_second")],by.x="f_eid",by.y="eid",all.x=T)
#Set NA to 0 for all subjects
bd[,copd_prim:=ifelse(is.na(copd_prim),0,copd_prim)]
bd[,asthma_prim:=ifelse(is.na(asthma_prim),0,asthma_prim)]
...
bd[,specialRequest_BIN_copdMappingStrict:=ifelse(copd_prim==1|f_20002_0_dxCode1112_BIN_chronic_obstructive_airways_disease_copd==1|specialRequest_BIN_copdBySpirOnlyStrict==1,1,ifelse(copd_prim==0&f_20002_0_dxCode1112_BIN_chronic_obstructive_airways_disease_copd==0&specialRequest_BIN_copdBySpirOnlyStrict==0,0,NA))]
bd[,specialRequest_BIN_copdMappingStrict_tobin_derived:=ifelse(copd_prim==1|f_20002_0_dxCode1112_BIN_chronic_obstructive_airways_disease_copd==1|specialRequest_BIN_copdBySpirOnlyStrict_tobin_derived==1,1,ifelse(copd_prim==0&f_20002_0_dxCode1112_BIN_chronic_obstructive_airways_disease_copd==0&specialRequest_BIN_copdBySpirOnlyStrict_tobin_derived==0,0,NA))]

#Exclude Asthma subjects based only on primary HES diagnoses as well as self-reported asthma (conversations with Dan and Mathias)
bd[,specialRequest_BIN_copdMappingLoose_noAsthma:=ifelse((copd_prim==1|f_20002_0_dxCode1112_BIN_chronic_obstructive_airways_disease_copd==1|specialRequest_BIN_copdBySpirOnlyLoose==1)&(f_20002_0_dxCode1111_BIN_asthma!=1&f_6152_0_code8_BIN_Asthma!=1&asthma_prim!=1),1,ifelse(copd_prim==0&f_20002_0_dxCode1112_BIN_chronic_obstructive_airways_disease_copd==0&specialRequest_BIN_copdBySpirOnlyLoose==0&(f_20002_0_dxCode1111_BIN_asthma!=1&f_6152_0_code8_BIN_Asthma!=1&asthma_prim!=1),0,NA))]
bd[,specialRequest_BIN_copdMappingStrict_noAsthma:=ifelse((copd_prim==1|f_20002_0_dxCode1112_BIN_chronic_obstructive_airways_disease_copd==1|specialRequest_BIN_copdBySpirOnlyStrict==1)&(f_20002_0_dxCode1111_BIN_asthma!=1&f_6152_0_code8_BIN_Asthma!=1&asthma_prim!=1),1,ifelse((copd_prim==0&f_20002_0_dxCode1112_BIN_chronic_obstructive_airways_disease_copd==0&specialRequest_BIN_copdBySpirOnlyStrict==0)&(f_20002_0_dxCode1111_BIN_asthma!=1&f_6152_0_code8_BIN_Asthma!=1&asthma_prim!=1),0,NA))]
bd[,specialRequest_BIN_copdMappingStrict_noAsthma_tobin_derived:=ifelse((copd_prim==1|f_20002_0_dxCode1112_BIN_chronic_obstructive_airways_disease_copd==1|specialRequest_BIN_copdBySpirOnlyStrict_tobin_derived==1)&(f_20002_0_dxCode1111_BIN_asthma!=1&f_6152_0_code8_BIN_Asthma!=1&asthma_prim!=1),1,ifelse((copd_prim==0&f_20002_0_dxCode1112_BIN_chronic_obstructive_airways_disease_copd==0&specialRequest_BIN_copdBySpirOnlyStrict_tobin_derived==0)&(f_20002_0_dxCode1111_BIN_asthma!=1&f_6152_0_code8_BIN_Asthma!=1&asthma_prim!=1),0,NA))]

#Include COPD primary and secondary
bd[,specialRequest_BIN_copdMappingLoose_primAndSec:=ifelse(copd_prim_second==1|f_20002_0_dxCode1112_BIN_chronic_obstructive_airways_disease_copd==1|specialRequest_BIN_copdBySpirOnlyLoose==1,1,ifelse(copd_prim_second==0&f_20002_0_dxCode1112_BIN_chronic_obstructive_airways_disease_copd==0&specialRequest_BIN_copdBySpirOnlyLoose==0,0,NA))]
bd[,specialRequest_BIN_copdMappingStrict_primAndSec:=ifelse(copd_prim_second==1|f_20002_0_dxCode1112_BIN_chronic_obstructive_airways_disease_copd==1|specialRequest_BIN_copdBySpirOnlyStrict==1,1,ifelse(copd_prim_second==0&f_20002_0_dxCode1112_BIN_chronic_obstructive_airways_disease_copd==0&specialRequest_BIN_copdBySpirOnlyStrict==0,0,NA))]
bd[,specialRequest_BIN_copdMappingStrict_primAndSec_tobin_derived:=ifelse(copd_prim_second==1|f_20002_0_dxCode1112_BIN_chronic_obstructive_airways_disease_copd==1|specialRequest_BIN_copdBySpirOnlyStrict_tobin_derived==1,1,ifelse(copd_prim_second==0&f_20002_0_dxCode1112_BIN_chronic_obstructive_airways_disease_copd==0&specialRequest_BIN_copdBySpirOnlyStrict_tobin_derived==0,0,NA))]

#Include COPD primary and secondary, no asthma for each
bd[,specialRequest_BIN_copdMappingLoose_noAsthma_primAndSec:=ifelse((copd_prim_second==1|f_20002_0_dxCode1112_BIN_chronic_obstructive_airways_disease_copd==1|specialRequest_BIN_copdBySpirOnlyLoose==1)&(f_20002_0_dxCode1111_BIN_asthma!=1&f_6152_0_code8_BIN_Asthma!=1&asthma_prim_second!=1),1,ifelse(copd_prim_second==0&f_20002_0_dxCode1112_BIN_chronic_obstructive_airways_disease_copd==0&specialRequest_BIN_copdBySpirOnlyLoose==0&(f_20002_0_dxCode1111_BIN_asthma!=1&f_6152_0_code8_BIN_Asthma!=1&asthma_prim_second!=1),0,NA))]
...
copdPrimForExac <- hesDataPrim[diag %like% "J440"|diag %like% "J441"|diag %like% "J449",.(eid,admidate,diag,disdate,epidur,epistart,epiend,record_id)]
copdSecForExac <- hesDataSec[diag %like% "J440"|diag %like% "J441",]
copdSecondWithEpiData <- merge(copdSecForExac,hesDataPrim[,.(eid,record_id,admidate,disdate,epidur,epistart,epiend)],by=c("record_id","eid"))[!duplicated(record_id)]
copdSecondWithEpiData <- copdSecondWithEpiData[,.(eid,admidate,diag,disdate,epidur,epistart,epiend,record_id)]
#Merge primary and secondary and remove dupicate record IDs
copdPrimarySecondWithEpiData <- as.data.table(rbind(copdPrimForExac,copdSecondWithEpiData))[!duplicated(record_id)]
copdPrimarySecondWithEpiData[,diag3char:="J44"]
#Determine if an ED or admitting episode
copdPrimarySecondWithEpiData[,specialRequest_CAT_type_of_COPD_exacerbation:=ifelse(epidur>=1,2,1)]
#Reduce to episodes lasting more than a day to specify hospitalizing exacerbations(To be used for phenotype B)
copdPrimarySecondWithEpiData_hospital <- copdPrimarySecondWithEpiData[epidur>=1,]
#Phenotype A. hospitalizing exacerbations
#Reduce to subjects with unique episode start date to identify unique exacerbation; based on conversations with Mathias
#We want to prioritze hospitalizing visits; sort by longest duration
copdPrimarySecondWithEpiData_hospital <- copdPrimarySecondWithEpiData_hospital[order(eid,-epidur)]
copdPrimarySecondWithEpiData_hospital <- copdPrimarySecondWithEpiData_hospital[!duplicated(copdPrimarySecondWithEpiData_hospital,by=c("eid","epistart")),]
#With unique episodes, create the counts
#Default behavior to return counts of code per subject
copdPrimarySecondWithEpiData_hospital_Wide <-dcast(copdPrimarySecondWithEpiData_hospital, eid ~ diag3char, value.var = c("diag3char"))
names(copdPrimarySecondWithEpiData_hospital_Wide)=c("f_eid","specialRequest_QUANT_number_of_hospitalizing_copd_exacerbation")
...
# hesDataCOPDExacSecHannaDefinition <- hesDataSec[diag %like% "J440"|diag %like% "J441",]
# hesDataCOPDExacPrimarySecondaryHannaDefinition <- rbind(hesDataCOPDExacPrimHannaDefinition[,c(1,5)],hesDataCOPDExacSecHannaDefinition[,c(1,4)])
# hesDataCOPDExacPrimarySecondaryHannaDefinition<-hesDataCOPDExacPrimarySecondaryHannaDefinition[!duplicated(hesDataCOPDExacPrimarySecondaryHannaDefinition,by=c("eid")),]
...
#Fixed 2 way mapping based on discussion with Robert Scott
...
#Add in a pack years variable
bd[,specialRequest_QUANT_numCigarettes:=ifelse(f_20116_0_0_CAT_never_former_current_smoking_status==2,f_3456_0_0_f_QUANT_Number_of_cigarettes_currently_smoked_daily_current_cigarette_smokers,
  ifelse(f_20116_0_0_CAT_never_former_current_smoking_status==1|(f_20116_0_0_CAT_never_former_current_smoking_status==2&is.na(f_3456_0_0_f_QUANT_Number_of_cigarettes_currently_smoked_daily_current_cigarette_smokers)),f_2887_0_0_f_QUANT_Number_of_cigarettes_previously_smoked_daily,NA))]
#Code above sets a standard cigarette variable. Need to first finish coding cigarettes before coding pack years
bd[,specialRequest_QUANT_numCigarettes:= ifelse(specialRequest_QUANT_numCigarettes=="-10",1,ifelse(specialRequest_QUANT_numCigarettes=="-1",NA,specialRequest_QUANT_numCigarettes))]
#Create age started smoking
bd[,specialRequest_QUANT_age_started_smoking := ifelse(f_20116_0_0_CAT_never_former_current_smoking_status==2,f_3436_0_0_f_QUANT_Age_started_smoking_in_current_smokers,ifelse((f_20116_0_0_CAT_never_former_current_smoking_status==2&is.na(f_3436_0_0_f_QUANT_Age_started_smoking_in_current_smokers))|f_20116_0_0_CAT_never_former_current_smoking_status==1,f_2867_0_0_f_QUANT_Age_started_smoking_in_former_smokers,NA))]
#Create age stopped smoking
bd[,specialRequest_QUANT_age_stopped_smoking := ifelse(f_20116_0_0_CAT_never_former_current_smoking_status==2,f_21003_0_0_f_QUANT_Age_when_attended_assessment_centre,ifelse(f_20116_0_0_CAT_never_former_current_smoking_status==1|(f_20116_0_0_CAT_never_former_current_smoking_status==2&!is.na(f_2897_0_0_f_QUANT_Age_stopped_smoking)),f_2897_0_0_f_QUANT_Age_stopped_smoking,NA))]
#Set NAs where appropriate
bd[,specialRequest_QUANT_age_stopped_smoking:=ifelse(specialRequest_QUANT_age_stopped_smoking<0,NA,specialRequest_QUANT_age_stopped_smoking)]
bd[,specialRequest_QUANT_pack_years:= (specialRequest_QUANT_numCigarettes/20)*(specialRequest_QUANT_age_stopped_smoking-specialRequest_QUANT_age_started_smoking)]
#Create a list of the variables that we want to keep or exclude
...
#Generate the set of variables that overlap between the bd and priority list
...
#covarFiles[,subjectsForBOLT_analysis_BIN:=1]
#Merge in Ioanna's covar file
...
for(phenoVariable in names(bd_residualization)[3:ncol(bd_residualization)]){
  ...
  bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=NA]
  bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=as.double(resid(residModel))]
  bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=qnorm((rank(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]],na.last="keep")/sum(!is.na(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]]))))]
  set(bd_residualization,i=which(is.infinite(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]])),j=paste0(phenoVariable,"_residualizedRelated"),value=NA)
  print(phenoVariable)
},error=function(e){print(paste0("Error with:",phenoVariable))})}

#Overwrite residualized variables where necessary with appropriate model
#We decided to not have any special residualizations
# #Ejection Fraction
# phenoVariable="f_22420_2_0_f_QUANT_LV_ejection_fraction"
# residModel=lm(data=bd_residualization,bd_residualization[[phenoVariable]]~age+sex+chip+f_50_0_0_f_QUANT_Standing_height+f_21002_0_0_f_QUANT_Weight,na.action=na.exclude)
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=NA]
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=as.double(resid(residModel))]
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=qnorm((rank(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]],na.last="keep")/sum(!is.na(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]]))))]
# set(bd_residualization,i=which(is.infinite(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]])),j=paste0(phenoVariable,"_residualizedRelated"),value=NA)
# #FEV1
# #Requires age squared variable
bd_residualization[,specialRequest_ageSquared_QUANT_age_squared := age^2]
# phenoVariable="f_3063_0_f_QUANT_FEV1_maximumValue"
# residModel=lm(data=bd_residualization,bd_residualization[[phenoVariable]]~f_20160_0_0_f_BIN_Ever_smoked+age+specialRequest_ageSquared_QUANT_age_squared+f_50_0_0_f_QUANT_Standing_height+sex+chip,na.action=na.exclude)
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=NA]
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=as.double(resid(residModel))]
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=qnorm((rank(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]],na.last="keep")/sum(!is.na(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]]))))]
# set(bd_residualization,i=which(is.infinite(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]])),j=paste0(phenoVariable,"_residualizedRelated"),value=NA)
# #FVC
# #Requires age squared variable
# phenoVariable="f_3062_0_f_QUANT_FVC_maximumValue"
# residModel=lm(data=bd_residualization,bd_residualization[[phenoVariable]]~f_20160_0_0_f_BIN_Ever_smoked+age+specialRequest_ageSquared_QUANT_age_squared+f_50_0_0_f_QUANT_Standing_height+sex+chip,na.action=na.exclude)
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=NA]
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=as.double(resid(residModel))]
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=qnorm((rank(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]],na.last="keep")/sum(!is.na(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]]))))]
# set(bd_residualization,i=which(is.infinite(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]])),j=paste0(phenoVariable,"_residualizedRelated"),value=NA)
# #FVC with ERS ATS filter
# #Requires age squared variable
# phenoVariable="f_3062_0_f_QUANT_FVC_maximumValue_strict"
# residModel=lm(data=bd_residualization,bd_residualization[[phenoVariable]]~f_20160_0_0_f_BIN_Ever_smoked+age+specialRequest_ageSquared_QUANT_age_squared+f_50_0_0_f_QUANT_Standing_height+sex+chip,na.action=na.exclude)
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=NA]
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=as.double(resid(residModel))]
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=qnorm((rank(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]],na.last="keep")/sum(!is.na(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]]))))]
...
# #FEV1/FVC ratio
# #Requires age squared variable
# phenoVariable="specialRequest_QUANT_FEV1_FVC_ratio"
# residModel=lm(data=bd_residualization,bd_residualization[[phenoVariable]]~f_20160_0_0_f_BIN_Ever_smoked+age+specialRequest_ageSquared_QUANT_age_squared+f_50_0_0_f_QUANT_Standing_height+sex+chip,na.action=na.exclude)
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=NA]
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=as.double(resid(residModel))]
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=qnorm((rank(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]],na.last="keep")/sum(!is.na(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]]))))]
# set(bd_residualization,i=which(is.infinite(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]])),j=paste0(phenoVariable,"_residualizedRelated"),value=NA)
# #FEV1/FVC smoked only ratio
# #Requires age squared variable
# phenoVariable="specialRequest_QUANT_FEV1_ever_smoked_only"
# residModel=lm(data=bd_residualization,bd_residualization[[phenoVariable]]~age+specialRequest_ageSquared_QUANT_age_squared+f_50_0_0_f_QUANT_Standing_height+sex+chip,na.action=na.exclude)
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=NA]
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=as.double(resid(residModel))]
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=qnorm((rank(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]],na.last="keep")/sum(!is.na(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]]))))]
# set(bd_residualization,i=which(is.infinite(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]])),j=paste0(phenoVariable,"_residualizedRelated"),value=NA)
# #Asthma(no age covariate to be used)
# phenoVariable="f_20009_0_dxAgeCode1111_QUANT_asthma"
# residModel=lm(data=bd_residualization,bd_residualization[[phenoVariable]]~f_20160_0_0_f_BIN_Ever_smoked+sex+chip,na.action=na.exclude)
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=NA]
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=as.double(resid(residModel))]
# bd_residualization[,paste0(phenoVariable,"_residualizedRelated"):=qnorm((rank(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]],na.last="keep")/sum(!is.na(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]]))))]
# set(bd_residualization,i=which(is.infinite(bd_residualization[[paste0(phenoVariable,"_residualizedRelated")]])),j=paste0(phenoVariable,"_residualizedRelated"),value=NA)

#Create a variable to represent a random splitting of data
bd[IID %in% sample(bd[["IID"]],(nrow(bd)/2)),UKB_split_BIN_randomly_split_cohort_in_half:= 1]
bd[,UKB_split_BIN_randomly_split_cohort_in_half := ifelse(is.na(UKB_split_BIN_randomly_split_cohort_in_half),0,UKB_split_BIN_randomly_split_cohort_in_half)]
#Remove the unnecessary columns
bd_residualization<-bd_residualization[,c("FID","IID",grep("^f_",names(bd_residualization),value=T),grep("specialRequest",names(bd_residualization),value=T),"specialRequest_ageSquared_QUANT_age_squared"),with=FALSE]
#Remove the binary smoke variable that got residualized as aprt of residualization loop
bd_residualization[,f_20160_0_0_f_BIN_Ever_smoked_residualizedRelated:=NULL]
#Merge in the residualized variables
bd <- merge(bd,bd_residualization[,c("FID","IID",grep("_residualizedRelated",names(bd_residualization),value=T),"specialRequest_ageSquared_QUANT_age_squared"),with=FALSE],by=c("FID","IID"),all.x=T)
#Remove HES cancer codes from phenotype file
#hesCancerRemove <- fread("/GWD/appbase/projects/RD-TSci-PhewasUKB/josh/benchmarking/codeTesting/removeHES_cancers.ls",header=F)
#hesCancerRemove <- hesCancerRemove[,! V1 %in% bdNames]

#Generate ARDB2 phenotype
#For phenotypes B and C, only focus on hospitalizing asthma
#Episode; A continuous period of admitted patient care administered under one consultant within one healthcare provider. If a patient is transferred to another consultant or to a different healthcare provider during a spell of treatment, a new record is generated, i.e. a new episode.
hesDataPrim <- fread("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/dependencies/HESIN_26041_all.minus4226129.tsv",header=T,sep="\t",na.strings="")
names(hesDataPrim)[5]="diag"
#f_34_0_0_f_QUANT_Year_of_birth
hesDataPrimARBD2 <- hesDataPrim[diag %like% "J45",.(eid,admidate,diag,disdate,epidur,epistart,epiend)]
hesDataPrimARBD2[,diag3char:="J45"]
#Generate set variable representing ED or hospitalization visit using admission information
#Phenotype A. hospitalizing exacerbations
hesDataPrimARBD2[,specialRequest_QUANT_type_of_asthma_exacerbation:=ifelse(epidur>=1,2,1)]
hesDataPrimARBD2 <- hesDataPrimARBD2[order(eid,-epidur)]
hesDataPrimARBD2noDupes <- hesDataPrimARBD2[!duplicated(hesDataPrimARBD2,by=c("eid")),]
#Reduce to episodes lasting more than a day to specify hospitalizing exacerbations(To be used for phenotype B)
hesDataPrimARBD2_hospital <- hesDataPrimARBD2[epidur>=1,]
#Reduce to subjects with unique episode start date to identify unique exacerbation; based on conversations with Mathias
#We want to prioritze hospitalizing visits; sort by longest duration
hesDataPrimARBD2_hospital <- hesDataPrimARBD2_hospital[order(eid,-epidur)]
hesDataPrimARBD2noDupes_hospital <- hesDataPrimARBD2_hospital[!duplicated(hesDataPrimARBD2_hospital,by=c("eid","epistart")),]
#With unique episodes, create the counts
#Default behavior to return counts of code per subject
hesDataPrimARBD2noDupesWide <-dcast(hesDataPrimARBD2noDupes_hospital, eid ~ diag3char, value.var = c("diag3char"))
#Create length of total hospital range using first diagnosis of any disease and last diagnosis of any disease
#Change this to using J45 to set observation
subjectLastAdmin <- hesDataPrim[order(eid,-epiend)][!duplicated(eid)][,.(eid,epiend)]
subjectFirstAdmin <- hesDataPrim[order(eid,epistart)][!duplicated(eid)][,.(eid,epistart)]
subjectFirstAndLast <- merge(subjectFirstAdmin,subjectLastAdmin)
#Create total number of years observed
subjectFirstAndLast[,specialRequest_QUANT_years_observed_in_primary_HES:= (as.Date(epiend)- as.Date(epistart))/365]
#If years observed is 0 then divide 1 day by 365
subjectFirstAndLast[,specialRequest_QUANT_years_observed_in_primary_HES:= ifelse(specialRequest_QUANT_years_observed_in_primary_HES==0.000000000,1/365,specialRequest_QUANT_years_observed_in_primary_HES)]
#merge in years observed to full phenotype file
bd <- merge(bd,subjectFirstAndLast[,.(eid,specialRequest_QUANT_years_observed_in_primary_HES)],by.x="IID",by.y="eid",all.x=T)
#For subjects that have only 1 visit in HES primary than set observed time to 1/365
bd[is.na(specialRequest_QUANT_years_observed_in_primary_HES),specialRequest_QUANT_years_observed_in_primary_HES:=1/365]
#For flexibility, a raw counts variable and annual rate variable will be created
hesDataPrimARBD2noDupesWide[,specialRequest_QUANT_asthma_hospital_exacerbations_raw:=J45]
#Merge in phenotype A into full matrix
bd <- merge(bd,hesDataPrimARBD2noDupes[,.(eid,specialRequest_QUANT_type_of_asthma_exacerbation)],by.x="IID",by.y="eid",all.x=T)
#Using predefined definition finish coding phenotype A
bd[,specialRequest_QUANT_type_of_asthma_exacerbation:=ifelse(is.na(specialRequest_QUANT_type_of_asthma_exacerbation)&map2way_NI_code1111_BIN_asthma==1,0,specialRequest_QUANT_type_of_asthma_exacerbation)]
#merge in raw hospitalizing counts and then set asthma cases without hospitalizing exacerbations to 0
bd <- merge(bd,hesDataPrimARBD2noDupesWide[,.(eid,specialRequest_QUANT_asthma_hospital_exacerbations_raw)],by.x="IID",by.y="eid",all.x=T)
bd[,specialRequest_QUANT_asthma_hospital_exacerbations_raw:=ifelse(is.na(specialRequest_QUANT_asthma_hospital_exacerbations_raw)&map2way_NI_code1111_BIN_asthma==1,0,specialRequest_QUANT_asthma_hospital_exacerbations_raw)]
#Create annual rate of asthma exacerbation
bd[!is.na(specialRequest_QUANT_asthma_hospital_exacerbations_raw)&!is.na(specialRequest_QUANT_years_observed_in_primary_HES),specialRequest_QUANT_asthma_hospital_exacerbations_annual_rate:=specialRequest_QUANT_asthma_hospital_exacerbations_raw/specialRequest_QUANT_years_observed_in_primary_HES]
#Age of onset
#Take earliest admission date per subject
hesDataPrimARBD2 <- hesDataPrimARBD2[order(eid,epistart)]
hesDataPrimARBD2noDupes_AOE <- hesDataPrimARBD2[!duplicated(hesDataPrimARBD2,by=c("eid")),]
hesDataPrimARBD2noDupes_AOE[,diagYear := as.numeric(format(as.Date(epistart),'%Y'))]
#Calculate HES based age of onset
hesDataPrimARBD2noDupes_AOE <- merge(bd[,.(IID,f_34_0_0_f_QUANT_Year_of_birth)],hesDataPrimARBD2noDupes_AOE,by.x="IID",by.y="eid")
hesDataPrimARBD2noDupes_AOE[,specialRequest_QUANT_asthma_HES_based_age_of_onset:=diagYear-f_34_0_0_f_QUANT_Year_of_birth]
#Merge in HES age for OA subjects
bd <- merge(bd,hesDataPrimARBD2noDupes_AOE[,.(IID,specialRequest_QUANT_asthma_HES_based_age_of_onset)],by="IID",all.x=T)
bd[,specialRequest_QUANT_asthma_HES_or_NI_based_age_of_onset_youngest:= apply(bd[,.(specialRequest_QUANT_asthma_HES_based_age_of_onset,f_20009_0_dxAgeCode1111_QUANT_asthma,f_22147_0_0_f_QUANT_Age_asthma_diagnosed_by_doctor,f_3786_0_0_f_QUANT_Age_asthma_diagnosed)],1,min,na.rm=T)]
set(bd,i=which(is.infinite(bd[["specialRequest_QUANT_asthma_HES_or_NI_based_age_of_onset_youngest"]])),j="specialRequest_QUANT_asthma_HES_or_NI_based_age_of_onset_youngest",value=NA)

#OA under 50
#get all the diganoses for OA
hesDataPrim <- fread("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/dependencies/HESIN_26041_all.minus4226129.tsv",header=T,sep="\t",na.strings="")
hesDataSec <- fread("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/dependencies/HESIN_SECONDARY_DIAG10_26041_all.minus508567.tsv",header=T,sep="\t",na.strings="")
names(hesDataPrim)[5]="diag"
names(hesDataSec)[4]="diag"
hesDataPrimSecMerged <- merge(hesDataPrim,hesDataSec,by="record_id",all.x=T)[,.(record_id,eid.x,epistart,diag.x,disdate,diag.y)]
hesDataPrimSecMerged_OA<-hesDataPrimSecMerged[diag.x %like% "M15"|diag.x %like% "M16"|diag.x %like% "M17"|diag.x %like% "M18"|diag.x %like% "M19"|diag.y %like% "M15"|diag.y %like% "M16"|diag.y %like% "M17"|diag.y %like% "M18"|diag.y %like% "M19"]
#Take earliest admission date per subject
hesDataPrimSecMerged_OA <- hesDataPrimSecMerged_OA[order(eid.x,epistart)][!duplicated(eid.x)]
hesDataPrimSecMerged_OA[,diagYear := as.numeric(format(as.Date(epistart),'%Y'))]
#Calculate HES based age of onset
hesDataPrimSecMerged_OA <- merge(bd[,.(IID,f_34_0_0_f_QUANT_Year_of_birth)],hesDataPrimSecMerged_OA,by.x="IID",by.y="eid.x")
hesDataPrimSecMerged_OA[,specialRequest_QUANT_OA_HES_based_age_of_onset:=diagYear-f_34_0_0_f_QUANT_Year_of_birth]
#Merge in HES age for OA subjects
...
bd[,f_2956_0_0_f_BIN_General_pain_for_3_months_vs_no_pain:=ifelse(f_6159_code8_BIN_Pain_all_over_the_body==0,0,ifelse(f_2956_0_0_f_BIN_General_pain_for_3_months==1,1,NA))]
bd[,f_3404_0_0_f_BIN_Neck_shoulder_pain_for_3_months_vs_no_pain:=ifelse(f_6159_code3_BIN_Neck_or_shoulder_pain==0,0,ifelse(f_3404_0_0_f_BIN_Neck_shoulder_pain_for_3_months==1,1,NA))]
bd[,f_3414_0_0_f_BIN_Hip_pain_for_3_months_vs_no_pain:=ifelse(f_6159_code6_BIN_Hip_pain==0,0,ifelse(f_3414_0_0_f_BIN_Hip_pain_for_3_months==1,1,NA))]
bd[,f_3571_0_0_f_BIN_Back_pain_for_3_months_vs_no_pain:=ifelse(f_6159_code4_BIN_Back_pain==0,0,ifelse(f_3571_0_0_f_BIN_Back_pain_for_3_months==1,1,NA))]
bd[,f_3741_0_0_f_BIN_Stomach_abdominal_pain_for_3_months_vs_no_pain:=ifelse(f_6159_code5_BIN_Stomach_or_abdominal_pain==0,0,ifelse(f_3741_0_0_f_BIN_Stomach_abdominal_pain_for_3_months==1,1,NA))]
bd[,f_3773_0_0_f_BIN_Knee_pain_for_3_months_vs_no_pain:=ifelse(f_6159_code7_BIN_Knee_pain==0,0,ifelse(f_3773_0_0_f_BIN_Knee_pain_for_3_months==1,1,NA))]
bd[,f_3799_0_0_f_BIN_Headaches_for_3_months_vs_no_pain:=ifelse(f_6159_code1_BIN_Headache==0,0,ifelse(f_3799_0_0_f_BIN_Headaches_for_3_months==1,1,NA))]
bd[,f_4067_0_0_f_BIN_Facial_pains_for_3_months_vs_no_pain:=ifelse(f_6159_code2_BIN_Facial_pain==0,0,ifelse(f_4067_0_0_f_BIN_Facial_pains_for_3_months==1,1,NA))]
bd[,specialRequest_BIN_joint_pain_versus_no_pain_union := ifelse(f_3414_0_0_f_BIN_Hip_pain_for_3_months_vs_no_pain==1|f_3404_0_0_f_BIN_Neck_shoulder_pain_for_3_months_vs_no_pain==1|f_3773_0_0_f_BIN_Knee_pain_for_3_months_vs_no_pain==1,1,ifelse(f_3414_0_0_f_BIN_Hip_pain_for_3_months_vs_no_pain==0 & f_3404_0_0_f_BIN_Neck_shoulder_pain_for_3_months_vs_no_pain==0&f_3773_0_0_f_BIN_Knee_pain_for_3_months_vs_no_pain==0,0,NA))]
#Same as above but keep with super controls and super cases. This is a similar definition to general pain
bd[,specialRequest_BIN_joint_pain_versus_no_pain_intersection := ifelse(f_3414_0_0_f_BIN_Hip_pain_for_3_months_vs_no_pain==1&f_3404_0_0_f_BIN_Neck_shoulder_pain_for_3_months_vs_no_pain==1&f_3773_0_0_f_BIN_Knee_pain_for_3_months_vs_no_pain==1,1,ifelse(f_3414_0_0_f_BIN_Hip_pain_for_3_months_vs_no_pain==0 & f_3404_0_0_f_BIN_Neck_shoulder_pain_for_3_months_vs_no_pain==0&f_3773_0_0_f_BIN_Knee_pain_for_3_months_vs_no_pain==0,0,NA))]
#Create an any pain 3+ month analysis
bd[,specialRequest_any_pain_for_3_months_vs_no_pain:=ifelse(f_3571_0_0_f_BIN_Back_pain_for_3_months_vs_no_pain==0&f_2956_0_0_f_BIN_General_pain_for_3_months_vs_no_pain==0&f_3414_0_0_f_BIN_Hip_pain_for_3_months_vs_no_pain==0&f_3773_0_0_f_BIN_Knee_pain_for_3_months_vs_no_pain==0&f_3404_0_0_f_BIN_Neck_shoulder_pain_for_3_months_vs_no_pain==0,0,ifelse(f_3571_0_0_f_BIN_Back_pain_for_3_months_vs_no_pain==1|f_2956_0_0_f_BIN_General_pain_for_3_months_vs_no_pain==1|f_3414_0_0_f_BIN_Hip_pain_for_3_months_vs_no_pain==1|f_3773_0_0_f_BIN_Knee_pain_for_3_months_vs_no_pain==1|f_3404_0_0_f_BIN_Neck_shoulder_pain_for_3_months_vs_no_pain==1,1,NA))]

#Merge 6153 and 6177 (medications when seperated by men and woman)
bd[,f_6153_6177_0_code1_BIN_Cholesterol_lowering_medication:=
ifelse(is.na(f_6177_0_code1_BIN_Cholesterol_lowering_medication)&is.na(f_6153_0_code1_BIN_Cholesterol_lowering_medication),NA,
  ifelse(is.na(f_6177_0_code1_BIN_Cholesterol_lowering_medication)&f_6153_0_code1_BIN_Cholesterol_lowering_medication==0,0,
    ifelse(is.na(f_6177_0_code1_BIN_Cholesterol_lowering_medication)&f_6153_0_code1_BIN_Cholesterol_lowering_medication==1,1,
      ifelse(is.na(f_6153_0_code1_BIN_Cholesterol_lowering_medication)&f_6177_0_code1_BIN_Cholesterol_lowering_medication==0,0,
        ifelse(is.na(f_6153_0_code1_BIN_Cholesterol_lowering_medication)&f_6177_0_code1_BIN_Cholesterol_lowering_medication==1,1,NA)))))]

bd[,f_6153_6177_0_code2_BIN_Blood_pressure_medication:=
ifelse(is.na(f_6177_0_code2_BIN_Blood_pressure_medication)&is.na(f_6153_0_code2_BIN_Blood_pressure_medication),NA,
  ifelse(is.na(f_6177_0_code2_BIN_Blood_pressure_medication)&f_6153_0_code2_BIN_Blood_pressure_medication==0,0,
    ifelse(is.na(f_6177_0_code2_BIN_Blood_pressure_medication)&f_6153_0_code2_BIN_Blood_pressure_medication==1,1,
      ifelse(is.na(f_6153_0_code2_BIN_Blood_pressure_medication)&f_6177_0_code2_BIN_Blood_pressure_medication==0,0,
        ifelse(is.na(f_6153_0_code2_BIN_Blood_pressure_medication)&f_6177_0_code2_BIN_Blood_pressure_medication==1,1,NA)))))]

bd[,f_6153_6177_0_code3_BIN_Insulin:=
ifelse(is.na(f_6177_0_code3_BIN_Insulin)&is.na(f_6153_0_code3_BIN_Insulin),NA,
  ifelse(is.na(f_6177_0_code3_BIN_Insulin)&f_6153_0_code3_BIN_Insulin==0,0,
    ifelse(is.na(f_6177_0_code3_BIN_Insulin)&f_6153_0_code3_BIN_Insulin==1,1,
      ifelse(is.na(f_6153_0_code3_BIN_Insulin)&f_6177_0_code3_BIN_Insulin==0,0,
        ifelse(is.na(f_6153_0_code3_BIN_Insulin)&f_6177_0_code3_BIN_Insulin==1,1,NA)))))]

#Merge in medication data
medicationData <- fread("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/dependencies/med_phenotypes_level3.tsv",sep="\t")
names(medicationData)[3:ncol(medicationData)]=paste0(names(medicationData)[3:ncol(medicationData)],"_BIN_medications")
...
#Remove other malignant neoplasms (Suggestions from Robert S)
bd[,f_40006_0_p_C44_BIN_Other_malignant_neoplasms_of_skin:=NULL]
#Any case from all cancer registry
bd[,f_40006_0_p_anyCancer_from_cancer_reg:=0]
set(bd,i=which(apply(bd[,names(bd) %like% "f_40006_0_p",with=FALSE]==1,1,any)),j="f_40006_0_p_anyCancer_from_cancer_reg",value=1)
#Manually rename a few of the variables that we created cases/control from date information
names(bd)=gsub(pattern="_pheno_f_DATE_Date_of_first","",names(bd))
#Modify cognitive function
bd[,f_20016_20191_0_0_f_QUANT_Fluid_intelligence_score:= ifelse(is.na(f_20016_0_0_f_QUANT_Fluid_intelligence_score),f_20191_0_0_f_QUANT_Fluid_intelligence_score,f_20016_0_0_f_QUANT_Fluid_intelligence_score)]
bd[,f_4282_20240_0_0_f_QUANT_Maximum_digits_remembered_correctly:=ifelse(is.na(f_4282_0_0_f_QUANT_Maximum_digits_remembered_correctly),f_20240_0_0_f_QUANT_Maximum_digits_remembered_correctly,f_4282_0_0_f_QUANT_Maximum_digits_remembered_correctly)]
bd[,f_4282_20240_0_0_f_QUANT_Maximum_digits_remembered_correctly:=ifelse(f_4282_20240_0_0_f_QUANT_Maximum_digits_remembered_correctly=="-1",NA,f_4282_20240_0_0_f_QUANT_Maximum_digits_remembered_correctly)]
#Pediatric Asthma set cases to missing if Asthma diagnosis is greater than 16, otherwise draw upon NI asthma DX
bd[,specialRequest_BIN_NI_pediatric_asthma_under16yo:=ifelse(is.na(f_20009_0_dxAgeCode1111_QUANT_asthma),f_20002_0_dxCode1111_BIN_asthma,
  ifelse(f_20009_0_dxAgeCode1111_QUANT_asthma>16&f_20002_0_dxCode1111_BIN_asthma==1,NA,f_20002_0_dxCode1111_BIN_asthma))]
#Adult only asthma
bd[,specialRequest_BIN_NI_adult_only_asthma_over16yo:=ifelse(is.na(f_20009_0_dxAgeCode1111_QUANT_asthma),f_20002_0_dxCode1111_BIN_asthma,
  ifelse(f_20009_0_dxAgeCode1111_QUANT_asthma<16&f_20002_0_dxCode1111_BIN_asthma==1,NA,f_20002_0_dxCode1111_BIN_asthma))]
#Binary version of 40009
bd[,f_40009_0_0_f_BIN_Reported_occurrences_of_cancer:=ifelse(is.na(f_40009_0_0_f_QUANT_Reported_occurrences_of_cancer),0,ifelse(f_40009_0_0_f_QUANT_Reported_occurrences_of_cancer>0,1,0))]
#Some of the cognitive variables need to be refined
#Used extreme controls based on all the criteria defined in "http://biobank.ctsu.ox.ac.uk/crystal/docs/MentalStatesDerivation.pdf"
bd[,f_20123_0_0_f_BIN_Single_episode_of_probable_major_depression:=ifelse(f_4598_0_0_f_BIN_Ever_depressed_for_a_whole_week==0&f_4631_0_0_f_BIN_Ever_unenthusiastic_disinterested_for_a_whole_week==0&f_2090_0_0_f_BIN_Seen_doctor_GP_for_nerves_anxiety_tension_or_depression==0&f_2100_0_0_f_BIN_Seen_a_psychiatrist_for_nerves_anxiety_tension_or_depression==0,0,f_20123_0_0_f_BIN_Single_episode_of_probable_major_depression)]
bd[,f_20124_0_0_f_BIN_Probable_recurrent_major_depression_moderate:=ifelse(f_4598_0_0_f_BIN_Ever_depressed_for_a_whole_week==0&f_4631_0_0_f_BIN_Ever_unenthusiastic_disinterested_for_a_whole_week==0&f_2090_0_0_f_BIN_Seen_doctor_GP_for_nerves_anxiety_tension_or_depression==0&f_2100_0_0_f_BIN_Seen_a_psychiatrist_for_nerves_anxiety_tension_or_depression==0,0,f_20124_0_0_f_BIN_Probable_recurrent_major_depression_moderate)]
bd[,f_20125_0_0_f_BIN_Probable_recurrent_major_depression_severe:=ifelse(f_4598_0_0_f_BIN_Ever_depressed_for_a_whole_week==0&f_4631_0_0_f_BIN_Ever_unenthusiastic_disinterested_for_a_whole_week==0&f_2090_0_0_f_BIN_Seen_doctor_GP_for_nerves_anxiety_tension_or_depression==0&f_2100_0_0_f_BIN_Seen_a_psychiatrist_for_nerves_anxiety_tension_or_depression==0,0,f_20125_0_0_f_BIN_Probable_recurrent_major_depression_severe)]
#Introduce primary hypertension based on full ICD10 code
hesDataPrim <- fread("/GWD/appbase/projects/RD-TSci-PhewasUKB/josh/phenotypes/HESIN_26041_all.minus4226129.tsv",header=T,sep="\t",na.strings="")
hesDataSec <- fread("/GWD/appbase/projects/RD-TSci-PhewasUKB/josh/phenotypes/HESIN_SECONDARY_DIAG10_26041_all.minus508567.tsv",header=T,sep="\t",na.strings="")
names(hesDataPrim)[5]="diag"
names(hesDataSec)[4]="diag"
...
bd[,hes_primary_p_j45_BIN_asthma_select_controls:=ifelse(HES_primary_p_J45_BIN_Asthma==1,1,
  ifelse(is.na(f_6152_0_code8_BIN_Asthma)&is.na(f_22127_0_0_f_BIN_Doctor_diagnosed_asthma)&map2way_NI_code1111_BIN_asthma==0,0,
    ifelse(is.na(f_6152_0_code8_BIN_Asthma)&f_22127_0_0_f_BIN_Doctor_diagnosed_asthma==0&map2way_NI_code1111_BIN_asthma==0,0,
      ifelse(is.na(f_22127_0_0_f_BIN_Doctor_diagnosed_asthma)&f_6152_0_code8_BIN_Asthma==0&map2way_NI_code1111_BIN_asthma==0,0,
        ifelse(f_6152_0_code8_BIN_Asthma==0&f_22127_0_0_f_BIN_Doctor_diagnosed_asthma==0&map2way_NI_code1111_BIN_asthma==0,0,NA)))))]
#Case only phenotypes
#primary cases set to 1 and secondary and self-reported phenotyes set to 0
bd[,hes_primary_p_j45_BIN_asthma_prim_vs_sec_and_SR_case_only:=ifelse(HES_primary_p_J45_BIN_Asthma==1,1,
  ifelse(map2way_NI_code1111_BIN_asthma==1,0,
    ifelse(f_6152_0_code8_BIN_Asthma==1|f_22127_0_0_f_BIN_Doctor_diagnosed_asthma==1,0,NA)))]
...
write.table(names(bd),file=paste0("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/UKB9888.names.",myTimeStamp,".csv"),sep=",",quote=F)

#Remove non essential variables from full phentoype matrix before outputting to file but first 
bd<- bd[,nonPriorityPhenotypes$V1:=NULL]
...
#Since this is the case I decided to build an empty matrix and fill in columns individually based on category

binaryTraits=names(bd)[bd[,names(bd) %like% "_BIN"]]
quantitativeTraits=names(bd)[bd[,names(bd) %like% "_QUANT"]]
myTable<-matrix(nrow=length(binaryTraits)+length(quantitativeTraits),ncol=16)
colnames(myTable)=c("Field","Controls","Cases","Missing","Bolt_Controls","Bolt_Cases","Unrelated_Controls","Unrelated_Cases","Exome_Controls","Exome_Cases","Min","1stQu","Median","Mean","3rdQu","Max")
myTable[,"Field"]=c(binaryTraits,quantitativeTraits)
...
for(i in binaryTraits){
	if(length(bd[bd[[i]]==1,.N,by=i][["N"]])>0)
	myTable[i,"Cases"]=bd[bd[[i]]==1,.N,by=i][["N"]]
	else(myTable[i,"Cases"]=0)
}
print("Done with raw tabulation")
#Bolt tabulation
bd_bolt<- merge(bd,covarFiles,by=c("FID","IID"))
for(i in binaryTraits){
	if(length(bd_bolt[bd_bolt[[i]]==0,.N,by=i][["N"]])>0)
	myTable[i,"Bolt_Controls"]=bd_bolt[bd_bolt[[i]]==0,.N,by=i][["N"]]
	else(myTable[i,"Bolt_Controls"]=0)
}
for(i in binaryTraits){
	if(length(bd_bolt[bd_bolt[[i]]==1,.N,by=i][["N"]])>0)
	myTable[i,"Bolt_Cases"]=bd_bolt[bd_bolt[[i]]==1,.N,by=i][["N"]]
	else(myTable[i,"Bolt_Cases"]=0)
}
print("Done with BOLT tabulation")
rm(bd_bolt)
#Unrelated Tabulation
unrelatedSubjects <- fread("/GWD/appbase/projects/RD-TSci-UKB/500k_GWASdownload/DataQC/sample_id/data.fam")
bd_unrelated <- bd[IID %in% unrelatedSubjects[["V2"]],]
for(i in binaryTraits){
	if(length(bd_unrelated[bd_unrelated[[i]]==0,.N,by=i][["N"]])>0)
	myTable[i,"Unrelated_Controls"]=bd_unrelated[bd_unrelated[[i]]==0,.N,by=i][["N"]]
	else(myTable[i,"Unrelated_Controls"]=0)
}
for(i in binaryTraits){
	if(length(bd_unrelated[bd_unrelated[[i]]==1,.N,by=i][["N"]])>0)
	myTable[i,"Unrelated_Cases"]=bd_unrelated[bd_unrelated[[i]]==1,.N,by=i][["N"]]
	else(myTable[i,"Unrelated_Cases"]=0)
}
print("Done with unrelated tabulation")
rm(bd_unrelated)
#Exome Tabulation
exomeSamples <- fread("/home/akp98077/Phase_1_Regeneron_PID_List.txt")
bd_exome <- bd[IID %in% exomeSamples[["eid"]],]
for(i in binaryTraits){
	if(length(bd_exome[bd_exome[[i]]==0,.N,by=i][["N"]])>0)
	myTable[i,"Exome_Controls"]=bd_exome[bd_exome[[i]]==0,.N,by=i][["N"]]
	else(myTable[i,"Exome_Controls"]=0)
}
for(i in binaryTraits){
	if(length(bd_exome[bd_exome[[i]]==1,.N,by=i][["N"]])>0)
	myTable[i,"Exome_Cases"]=bd_exome[bd_exome[[i]]==1,.N,by=i][["N"]]
	else(myTable[i,"Exome_Cases"]=0)
}
print("Done with exome tabulation")
rm(bd_exome)
#Add quantitative phenotypes into mix
for(i in quantitativeTraits){
  mySummary=summary(bd[[i]])
  if(length(summary(bd[[i]]))==7)
  myTable[i,c("Min","1stQu","Median","Mean","3rdQu","Max","Missing")]=mySummary[c("Min.","1st Qu.","Median", "Mean", "3rd Qu.","Max.","NA's")]
  else(myTable[i,c("Min","1stQu","Median","Mean","3rdQu","Max")]=mySummary[c("Min.","1st Qu.","Median", "Mean", "3rd Qu.","Max.")])
  #myTable[i,Missing:="0"]
}
...
#Generate combined table CSV
write.csv(myTable,quote=F,file=paste0("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/UKB9888.ALL.tables.",myTimeStamp,".csv"))
...
#      #print(i)
#      names(combineTable) =c("FIELD","Controls","Cases","Missing")
#      write.csv(t(combineTable),quote=F,file=paste0("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/UKB9888.BIN.tables.",myTimeStamp,".csv"))
#    }
...
#write.csv(quantVariables,quote=F,file=paste0("/GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/UKB9888.QUANT.tables.",myTimeStamp,".csv"))
print("time to write files:Stop")
print(proc.time())

system(paste0("cp /GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/workingCode_phenotyping_v5.R /GWD/appbase/projects/RD-TSci-UKB/workingPhenotypes/UKB9888.ALL.",myTimeStamp,".Rscript"))
